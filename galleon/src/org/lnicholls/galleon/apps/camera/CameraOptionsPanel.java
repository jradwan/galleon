...
public class CameraOptionsPanel extends AppConfigurationPanel {
    private static Logger log = Logger.getLogger(CameraOptionsPanel.class.getName());

    public CameraOptionsPanel(AppConfiguration appConfiguration) {
        super(appConfiguration);
        setLayout(new GridLayout(0, 1));

        CameraConfiguration cameraConfiguration = (CameraConfiguration) appConfiguration;

        mTitleField = new JTextField(cameraConfiguration.getName());
        mNameField = new JTextField();
        mCameraField = new JComboBox();
        List cameras = CameraDevice.getCameras();
        for (int i = 0; i < cameras.size(); i++)
            mCameraField.addItem(new ComboWrapper((String) cameras.get(i), (String) cameras.get(i)));
        ...
        builder.addLabel("Camera", cc.xy(1, 9));
        builder.add(mCameraField, cc.xyw(3, 9, 1));

        mColumnValues = new ArrayList();
        int counter = 0;
        for (Iterator i = cameraConfiguration.getCameras().iterator(); i.hasNext();) {
        ...
        columnNames.add(1, "Camera");
        ArrayList fields = new ArrayList();
        fields.add(mNameField);
        fields.add(mCameraField);
    ...
    public void save() {
        CameraConfiguration cameraConfiguration = (CameraConfiguration) mAppConfiguration;
        cameraConfiguration.setName(mTitleField.getText());
        ...
        cameraConfiguration.setCameras(newItems);
    }
    ...
    private JComboBox mCameraField;
