...
public class JabberOptionsPanel extends AppConfigurationPanel {
	private static Logger log = Logger.getLogger(JabberOptionsPanel.class.getName());
	...
	public JabberOptionsPanel(AppConfiguration appConfiguration) {
		...
		mPasswordField = new JPasswordField(Tools.decrypt(jabberConfiguration.getPassword()));
	...
	public boolean valid() {
		...
		if (mServerField.getText().trim().length() == 0) {
			JOptionPane.showMessageDialog(this, "Invalid server.", "Error", JOptionPane.ERROR_MESSAGE);
			return false;
		}
	...
	public void save() {
		...
		jabberConfiguration.setPassword(Tools.encrypt(mPasswordField.getText()));
