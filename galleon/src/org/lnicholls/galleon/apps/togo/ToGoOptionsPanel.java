...
public class ToGoOptionsPanel extends AppConfigurationPanel {
	...
	public ToGoOptionsPanel(AppConfiguration appConfiguration) {
		...
		mShowStatsField
				.setToolTipText("Check to specify that ToGo statistics should be shown");
		mShowStatsField.setSelected(togoConfiguration.isShowStats());
		mSortCombo = new JComboBox();
		mSortCombo.addItem(new ComboWrapper("Alphabetic",
				ToGoConfiguration.SORT_ALPHA));
		mSortCombo.addItem(new ComboWrapper("Latest First",
				ToGoConfiguration.SORT_DATE_LATEST));
		mSortCombo.addItem(new ComboWrapper("Oldest First",
				ToGoConfiguration.SORT_DATE_OLDEST));
		String sort = togoConfiguration.getSort();
		if (sort == null)
			sort = ToGoConfiguration.SORT_DATE_LATEST;
		defaultCombo(mSortCombo, sort);
		FormLayout layout = new FormLayout(
				"right:pref, 3dlu, 50dlu:g, right:pref:grow", "pref, " + // general
						"9dlu, " + "pref, " + // title
						"3dlu, " + "pref, " + // share
						"9dlu, " + "pref, " + // Options
	...
	public void save() {
		...
		togoConfiguration.setSort(((NameValue) mSortCombo.getSelectedItem())
				.getValue());
	}
