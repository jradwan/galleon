...
public class Upcoming extends DefaultApplication {
		...
		protected void createRow(BView parent, int index) {
			...
			try {
				if (event.getStartDate() != null && event.getStartDate().trim().length() > 0
					...
					Date startDate = mEventDateFormat.parse(event.getStartDate());
				...
				if (event.getEndDate() != null && event.getEndDate().trim().length() > 0
					...
					Date endDate = mEventDateFormat.parse(event.getEndDate());
	...
	private static SimpleDateFormat mEventDateFormat = new SimpleDateFormat();

	private static SimpleDateFormat mEventTimeFormat = new SimpleDateFormat();
}