						...
						synchronized (buffer) {
							...
							if (anchorItem != null) {
								...
								if (anchorItemURL.getPath().startsWith("/TiVoConnect/GalleonRecordings/")) {
									path = anchorItemURL.getPath()
											.substring("/TiVoConnect/GalleonRecordings/".length());
								} else {
								...
								for (int i = 0; i < files.size(); i++) {
									...
									String filePath = file.getCanonicalPath().substring(
											directory.getCanonicalPath().length() + 1).replaceAll("\\\\", "/");
							...
							for (int i = start; i < limit; i++) {
								...
								if (nameFile.isFolder()) {
									...
									if (video == null || !file.getName().toLowerCase().endsWith(".tivo")) {
										...
										buffer.append("<CaptureDate>0x"
												+ Tools.dateToHex(new Date(file.lastModified())) + "</CaptureDate>\n");
										...
										if (video.getSeriesTitle() != null)
											...
											buffer.append("<Title>" + Tools.escapeXMLChars(video.getTitle())
													+ "</Title>\n");
										...
										if (video.getOriginalAirDate() != null)
											buffer.append("<CaptureDate>0x"
													+ Tools.dateToHex(video.getOriginalAirDate()) + "</CaptureDate>\n");
										else
										...
										if (video != null)
											buffer
													.append("<Description>"
															+ Tools.escapeXMLChars(video.getDescription())
															+ "</Description>\n");
										else
										...
										if (video.getStation() != null)
											buffer
													.append("<SourceStation>" + video.getStation()
															+ "</SourceStation>\n");
										else
