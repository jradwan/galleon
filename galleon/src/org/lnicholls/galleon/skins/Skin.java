...
public class Skin {
	...
	public Skin(String path) {
		...
		mSkinDescriptor = parse(new String(mSkinLoader.findResource("skin.xml")
				.toByteArray()));
	}
	private SkinDescriptor parse(String value) {
		try {
			...
			skinDescriptor.setReleaseDate(Tools.getAttribute(root,
					"releaseDate"));
			skinDescriptor.setDescription(Tools.getAttribute(root,
					"description"));
			skinDescriptor
					.setAuthorName(Tools.getAttribute(root, "authorName"));
			skinDescriptor.setAuthorEmail(Tools.getAttribute(root,
					"authorEmail"));
			skinDescriptor.setAuthorHomepage(Tools.getAttribute(root,
					"authorHomepage"));
			for (Iterator imageIterator = root.elementIterator("image"); imageIterator
					.hasNext();) {
			...
			for (Iterator colorIterator = root.elementIterator("color"); colorIterator
					.hasNext();) {
			...
			for (Iterator i = root.elementIterator("app"); i.hasNext();) {
				...
				for (Iterator imageIterator = appElement
						.elementIterator("image"); imageIterator.hasNext();) {
					Element element = (Element) imageIterator.next();
					Descriptor.Image image = new Descriptor.Image();
					image.setSource(Tools.getAttribute(element, "source"));
					image.setId(Tools.getAttribute(element, "id"));
				...
				for (Iterator colorIterator = appElement
						.elementIterator("color"); colorIterator.hasNext();) {
					Element element = (Element) colorIterator.next();
					Descriptor.Color color = new Descriptor.Color();
					color.setValue(Tools.getAttribute(element, "value"));
					color.setId(Tools.getAttribute(element, "id"));
				...
				for (Iterator screenIterator = appElement
						.elementIterator("screen"); screenIterator.hasNext();) {
					...
					for (Iterator imageIterator = screenElement
							.elementIterator("image"); imageIterator.hasNext();) {
						Element element = (Element) imageIterator.next();
						Descriptor.Image image = new Descriptor.Image();
						image.setSource(Tools.getAttribute(element, "source"));
					...
					for (Iterator colorIterator = screenElement
							.elementIterator("color"); colorIterator.hasNext();) {
						Element element = (Element) colorIterator.next();
						Descriptor.Color color = new Descriptor.Color();
						color.setValue(Tools.getAttribute(element, "value"));
	...
	public ByteArrayOutputStream getImage(String appId, String screenId,
			String id) {
